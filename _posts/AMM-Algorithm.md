...
当$t\geq 2$，那么 $(δ^s)^{2^{t−2}}
(mod p)\in \{ 1, −1\}$（虽然在是不知道这是为什么，可能是将他当成一个新的二次（非）剩余），引入$k_1=\{0, 1\}$，表示成
...
当 $(δ^s)^{2^{t−2}}
\equiv1(mod p)$，$k_1=0$，$k_1=1$

然后我们继续对上面的式子开方， $(δ^s)^{2^{t−3}}(\rho^s)^{2^{t-2}\cdot k_1}
\pmod{p}\in \{ 1, −1\}$，并引入$k_2$
